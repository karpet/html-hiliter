...
sub _init {
   ...
   # since &lt; or &gt; can be indexed as literal < and >, at least by SWISH-E
    ...
    $self->{EndBound} ||= join( '|',
        '\Z', '[<&]', $White_Space, '[^' . $self->{EndCharacters} . ']' );
    ...
    my $igf
        = $self->{IgnoreFirstChar} ? qr/[$self->{IgnoreFirstChar}]*/i : '';
...
sub _mytag {
    my ( $parser, $tag, $tagname, $offset, $length, $offset_end, $attr,
        $text )
        = @_;
    ...
    if ( $HTML::Tagset::isPhraseMarkup{$tagname} ) {
        ...
        # and don't highlight
        if ( $hiliter->{BufferLim}
            and length($buffer) > $hiliter->{BufferLim} )
        {
    ...
    if ( $tag eq 'head' ) {
        if ( $hiliter->{Print} ) {
            ...
            $hiliter->{Buffer} .= $hiliter->{StyleHead}
                if $hiliter->{StyleHead};
        }
    }
...
sub _mytext {
    ...
    $hiliter->{dtext}
        .= $dtext;    # remember decoded to eval before calling hilite()
...
sub Run {
    ...
    if ( -e $file_or_html )    # should handle files or filehandles
    {
        ...
        die
            "$file_or_html is neither a file nor a filehandle nor a scalar ref!\n";
    ...
    my $parser = new HTML::Parser(
        ...
        start_h       => [
            \&_mytag, 'self,tag,tagname,offset,length,offset_end,attr,text'
        ],
        end_h => [
            \&_mytag, 'self,tag,tagname,offset,length,offset_end,undef,text'
        ],
...
sub hilite {
    ...
    my $tagless = $self->{dtext}
        || $html;    # if we didn't have dtext, we weren't using HTML::Parser
...
Q: for my $q (@all_queries) {
        ...
        $count->{$q} = _count_instances( $self, $q, $tagless, $links, $re )
            || 0;
    ...
    unless ( $count or _check_count($count) ) {
        ...
        my @instances
            = sort keys %$count;    # sort is just to make debugging easier
    ...
    Q: for my $q (@instances) {
        ...
        R: for my $r ( keys %$real ) {
                ...
                print $OC . "REAL appears to be $r ($real->{$r} instances)",
                    $CC
                    if $Debug >= 1;
   ...
   # changed to more accurate submatch with \\3 - Sat Oct 23 22:51:59 CDT 2004
                ...
                R: for my $r ( keys %$link_plus_txt ) {
                  ...
                  # by the standard @instances
                        ...
                        if ( $ltext =~ m/$re/isx ) {
                            print $OC . "SKIPPING LINK as duplicate" . $CC
                                if $Debug >= 1;
                            $count->{$q}--;
                            next R;
                        }

                        print $OC . "REAL LINK appears to be $r", $CC
                            if $Debug >= 1;

                        push( @{ $q2real->{$q} }, $r )
                            while $link_plus_txt->{$r}--;

                    }
                }
            }

            $self->{Report}->{$q}->{Instances}
                += scalar( @{ $q2real->{$q} || [] } );

            print $OC
                . "before any hiliting, count for '$q' is $self->{Report}->{$q}->{Instances} "
                . $CC
                if $Debug >= 1;
...
sub _report {
    ...
    my $file = $self->{File}
        || '[unknown file]';    # if we eval \$file, we don't care.
    for ( keys %$count ) {
        next if $count->{$_} <= 0;
...
sub _make_styles_css {
    ...
    for (@$queries) {
        ...
        $self->{CTags}->{$_}
            = "</$tag>";    # this is always the same; should we bother?
...
sub _add_hilite_tags {
    ...
    my $pre_added
        = $prefixed =~ s($tag_regexp+)${nocolor}$close$1$open${color}g;
    ...
    while ( $html =~ m/($st_bound)($safe)($end_bound)/g ) {
        ...
        my $pos = pos($html);
        my $newstring
            = $s . $open . $color . $prefixed . $nocolor . $close . $e;
        substr( $html, $pos - $len, $len, $newstring );

        pos($html)
            = $pos 
            + length( $open . $color . $nocolor . $close ) 
            + $len_added - 1;
...
# but -g to get all instances
    ...
    $html
        = _clean_up_hilites( $self, $html, $q, $open, $close, $safe, $count );
...
sub _clean_up_hilites {
    ...
    print $OC
        . "BEFORE cleanup, HiLite Count for '$q' is $self->{Report}->{$q}->{HiLites}"
        . $CC
        if $Debug >= 1;

    # empty hilites are useless
    my $empty = ( $html =~ s,$open(?:\Q$color\E)(?:\Q$nocolor\E)$close,,sgi )
        || 0;

# to be safe: in some cases we might match against entities or within tag content.
    my $ent_split
        = ( $html
            =~ s/(&[\w#]*)$open(?:\Q$color\E)(${safe})(?:\Q$nocolor\E)$close([\w#]*;)/$1$2$3/igs
        ) || 0;

    my $tag_split = 0;
    while ( $html
        =~ m/(<[^<>]*)\Q$open\E(?:\Q$color\E)($safe)(?:\Q$nocolor\E)\Q$close\E([^>]*>)/gxsi
        )
    {
        ...
        $tag_split
            += ( $html
                =~ s/(<[^<>]*)\Q$open\E(?:\Q$color\E)($safe)(?:\Q$nocolor\E)\Q$close\E([^>]*>)/$1$2$3/gxsi
            );
    ...
    print
        "$OC AFTER cleanup, HiLite Count for '$q' is $self->{Report}->{$q}->{HiLites} $CC"
        if $Debug >= 1;
...
# word and the stemmed word.
    ...
    K: for ( keys %uniq ) {
        ...
        W: for my $w (@w) {
                ...
                $w = $f
                    . '*';  # add wildcard so that regexp gets built correctly
            }
            my $new = join ' ', @w;
            if ( $new ne $_ ) {
...
The Debug feature requires L<Data::Dump> when you run Report().
